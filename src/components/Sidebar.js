...
const Sidebar = () => {
  ...
  return (
    <div className={`${isOpen ? 'right-0' : '-right-full'} w-full bg-white fixed top-0 h-full shadow-2x1
   md:w-[35vw] x1:max-w-[30vw] transition-all duration-300 z-20 px-4 lg:px-
   [35px]`}
    >
      <div className='flexx items-center justify-between py-6
    border-b'>
      ...
      <div className='uppercase font-semibold'>
        <span className='mr-2'>Total: </span>$ {parseFloat
        (total).toFixed(2)}
      </div>
      <div 
      onClick={clearCart}
